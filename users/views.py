...
def view_myaccount(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return render(
                request, 'view_myaccount.html', {'profile': profile})
        except Profile.DoesNotExist:
...
def profile_form_success(request):
    user = request.user
    return render(
        request, 'profile_form_success.html', {'user': user})
...
def delete_account_confirmation(request):
    if request.method == 'POST':
        ...
        if confirm_delete == 'Yes':
            ...
            logout(request)
            messages.success(
                request, 'Your MyStompbox has been successfully deleted.')
...
def contact_seller(request, listing_id):
    ...
    return render(
        request, 'contact_seller.html', {'form': form, 'listing': listing})
